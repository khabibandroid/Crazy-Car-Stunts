...
public class SwipeControl : MonoBehaviour {
	...
	public static SwipeControl mee;
	...
	public int mouseStartArea = 0; //set to 1 if mouse starts clicking within touchRect
...
//		Menu_Handler._instance.Levels_sel = startValue;
//		startValue = Menu_Handler._instance.Levels_sel;
						...
						if(Input.GetMouseButtonUp(0)) 
						{
							if((tPos - mStartPos).sqrMagnitude < 25) 
							{
								if(clickEdgeToSwitch)
								{
									...
									{
										currentValue --;
//										Menu_Handler._instance.Levels_sel--;
//										Menu_Handler._instance.Level_Images[currentValue].gameObject.transform.localScale=new Vector3(33,33,33);
										if(currentValue < 0)
										{
											currentValue = 0;
										}
//										if(Menu_Handler._instance.Levels_sel<0)
//										{
//											Menu_Handler._instance.Levels_sel=0;
//										}
									}
...
//										if(Menu_Handler._instance.Levels_sel > maxValue)
//										{
//											Menu_Handler._instance.Levels_sel = maxValue;
//										}
									}
								}
							} 
...
//								if(Menu_Handler._instance.Levels_sel- (smoothValue + (smoothValue - GetAvgValue(prevSmoothValue))) > smoothDragOffset || Menu_Handler._instance.Levels_sel - (smoothValue + (smoothValue - GetAvgValue(prevSmoothValue))) < -smoothDragOffset)
//								{
//									//dragged beyond dragOffset to the right
//									Menu_Handler._instance.Levels_sel= (int) Mathf.Round(smoothValue + (smoothValue - GetAvgValue(prevSmoothValue)));
//									xVelocity = (smoothValue - GetAvgValue(prevSmoothValue)); // * -0.10 ;
//									if(Menu_Handler._instance.Levels_sel > maxValue) Menu_Handler._instance.Levels_sel = maxValue;
//									else if(Menu_Handler._instance.Levels_sel < 0) Menu_Handler._instance.Levels_sel = 0;
////									Menu_Handler._instance.Level_Images[Menu_Handler._instance.Levels_sel].gameObject.transform.localScale=new Vector3(33,33,33);
//								}
							}		
							mouseStartArea = 0;
						}
...
//		if(GUI.Button(new Rect(0,0,100,60),new GUIContent("RESET")))
//		{
//			print("GUI Button");
//			currentValue = 40;
//		}
//		if(GUI.Button(new Rect(100,0,100,60),new GUIContent("COME")))
//		{
//			print("GUI Button");
//			currentValue = 0;
//		}
